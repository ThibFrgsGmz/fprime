...
<xsl:template match="/">## CppLint <xsl:value-of select="//cpplint/@version"/> Summary
...
| --- | --- | --- | --- |
<xsl:for-each select="results//error">| <xsl:value-of select="@severity"/> | <xsl:value-of select="@file"/>:<xsl:value-of select="@line"/> | <xsl:value-of select="@id"/> | <xsl:value-of select="@msg"/> |
</xsl:for-each>
</xsl:if>
